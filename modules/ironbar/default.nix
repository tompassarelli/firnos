...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    home-manager.users.${username} = { config, ... }: {
      ...
      xdg.configFile = {
        "ironbar/config.toml".source = config.lib.file.mkOutOfStoreSymlink "${config.home.homeDirectory}/code/nixos-config/dotfiles/ironbar/config.toml";
        "ironbar/stylix.css".text = with config.lib.stylix.colors; ''
